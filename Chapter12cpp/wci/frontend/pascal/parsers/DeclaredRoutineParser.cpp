...
EnumSet<PascalTokenType> DeclaredRoutineParser::PARAMETER_SET;
EnumSet<PascalTokenType> DeclaredRoutineParser::LEFT_PAREN_SET;
EnumSet<PascalTokenType> DeclaredRoutineParser::RIGHT_PAREN_SET;
EnumSet<PascalTokenType> DeclaredRoutineParser::PARAMETER_FOLLOW_SET;
EnumSet<PascalTokenType> DeclaredRoutineParser::COMMA_SET;
    ...
    throw (string)
{
    ...
    if (routine_id->get_routine_code() == (RoutineCode) RT_FORWARD)
    {
        symtab_stack->push(routine_id->get_routine_symtab());
        forwarded = true;
    }
    else
    {
    ...
    {
        vector<SymTabEntry *> *subroutines = parent_id->get_routine_ids();
        ...
        && (to_lower(token->get_text()) == "forward"))
    {
        ...
        routine_id->set_routine_code((RoutineCode) RT_FORWARD);
    }
    else
    {
        routine_id->set_routine_code((RoutineCode) RT_DECLARED);
    ...
    throw (string)
{
    ...
    if (token->get_type() == (TokenType) PT_IDENTIFIER)
    {
        ...
        else if (routine_id->get_routine_code() != (RoutineCode) RT_FORWARD)
        {
            routine_id = nullptr;
...
void DeclaredRoutineParser::parse_formal_parameters(
        ...
        throw (string)
{
    vector<SymTabEntry *> *parms = routine_id->get_routine_parms();
    ...
    if (token->get_type() == (TokenType) PT_LEFT_PAREN)
    {
               ...
               || (token_type == (TokenType) PT_VAR))
        {
            ...
            for (SymTabEntry *parm : parm_sublist) parms->push_back(parm);
