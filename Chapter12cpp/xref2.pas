...
FUNCTION IsLetter(ch2 : char) : boolean;

    {Return true if the character is a letter, false otherwise.}

    BEGIN
        IsLetter :=    ((ch2 >= 'a') AND (ch2 <= 'z'))
                    OR ((ch2 >= 'A') AND (ch2 <= 'Z'));
    END;
...
FUNCTION ReadWord(VAR buffer : wordType) : boolean;
    ...
    BEGIN
        ...
        IF NOT done THEN BEGIN
            ...
            WHILE IsLetter(ch3) DO BEGIN
                IF charcount < MaxWordLength THEN BEGIN
                    IF (ch3 >= 'A') AND (ch3 <= 'Z') THEN BEGIN
                        ch3 := chr(ord(ch3) + (ord('a') - ord('A')));
                    END;
                    charcount := charcount + 1;
                    buffer[charcount] := ch3;
                END;
                ch3 := NextChar;
            END;
...
BEGIN {Xref}
    ...
    WHILE NOT (done OR wordTableFull OR numberTableFull) DO BEGIN
