    ...
    : icode(nullptr), symtab_stack(nullptr), backend(nullptr)
{
    ...
    Options::lines        = flags.find('l') != string::npos;
    Options::assign       = flags.find('a') != string::npos;
    Options::fetch        = flags.find('f') != string::npos;
    Options::call         = flags.find('c') != string::npos;
    Options::returnn      = flags.find('r') != string::npos;
...
void Pascal::message_received(Message& message)
{
    switch (message.type)
    {
        ...
        case AT_LINE:
        {
            printf(LINE_FORMAT.c_str(), message.line_number);
            break;
        }
        ...
        case FETCH:
        {
            printf(FETCH_FORMAT.c_str(), message.line_number,
                   message.content.fetch.variable_name,
                   message.content.fetch.value);
            break;
        }

        case CALL:
        {
            printf(CALL_FORMAT.c_str(), message.line_number,
                   message.content.call.routine_name);
            break;
        }

        case RETURN:
        {
            printf(RETURN_FORMAT.c_str(), message.line_number,
                   message.content.call.routine_name);
            break;
        }
